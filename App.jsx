...
export default function App() {
  ...
  // Step 1 — stop recording, transcribe, show for review
  ...
  const handleConfirmAndScore = async () => {
    ...
    if (currentPlayerIndex + 1 < currentPlayerOrder.length) {
      ...
      await speak(pack2.rounds[selectedRoundIndex].objection, "character", selectedPackId);
      setPhase(PHASE.PLAYER_RESPONSE);
    } else {
          ...
          <div className="player-inputs">
            {players.map((player, i) => (
              <div key={i} className="player-input-row">
                <div className="player-number" style={{ background: ["#FFD700", "#4ECDC4", "#FF6B6B"][i] }}>
                  P{i + 1}
                </div>
                <input
          ...
          {(isTranscribing || transcript) && (
            <div className="transcript-box">
              <div className="transcript-label">
                {isTranscribing ? "🎙️ TRANSCRIBING..." : "📝 RESPONSE CAPTURED — REVIEW BEFORE SUBMITTING"}
              </div>
              {isTranscribing && (
                <div className="transcribing-dots"><span /><span /><span /></div>
              )}
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sc-title { font-family: 'Bebas Neue', sans-serif; font-size: 20px; letter-spacing: 3px; color: var(--text); }
  ...
  .btn-sc-test {
    background: var(--dark2); border: 1px solid var(--border); border-radius: 10px;
    color: var(--text); font-family: 'Bebas Neue', sans-serif; font-size: 15px;
  ...
  .btn-submit {
    ...
    color: #000; padding: 14px 32px; cursor: pointer;
    font-family: 'Bebas Neue', sans-serif; font-size: 18px; letter-spacing: 2px;
    transition: all 0.2s; box-shadow: 0 4px 20px rgba(255,215,0,0.3);
  }
  .btn-submit:hover { transform: translateY(-2px); box-shadow: 0 8px 28px rgba(255,215,0,0.5); }
